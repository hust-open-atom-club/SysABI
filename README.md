...
FuzzAsterinas 是一个面向 `syzkaller` 程序的离线差分回放框架。它的目标不是直接做在线 coverage-guided fuzzing，而是把 Linux `*.syz` 程序整理为可重放、可构建、可执行、可比较的 testcase，然后在 `reference` 与 `candidate` 两侧顺序执行，收集 trace、归一化、做语义比较，并产出可复盘的报告。
...
├── configs/               # baseline / asterinas / asterinas_scml 配置
